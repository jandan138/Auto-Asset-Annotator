...
from .model import BaseModelEngine  # 导入模型引擎协议，用于模型推理
from .prompt import PromptFactory  # 导入 PromptFactory 类，用于生成提示词
from ..config.settings import Config  # 导入 Config 类，用于获取配置
from ..utils.file import (
    get_asset_images,
)  # 导入 get_asset_images 函数，用于获取资产图片
from ..utils.image import (
    concatenate_images,
)  # 导入 concatenate_images 函数，用于拼接图片（暂未启用）


class AnnotationPipeline:  # 定义 AnnotationPipeline 类，用于管理标注流程
    def __init__(self, config: Config, engine: BaseModelEngine):  # 初始化方法
        self.config = config  # 保存配置对象
        self.engine = engine  # 保存模型引擎对象

    def process_asset(
        self, asset_path: str, prompt_type: str = None
    ) -> Dict[str, Any]:  # 处理单个资产的方法
        """
        ...
        # 1. Find Images
        images_map = get_asset_images(
            asset_path, self.config.data
        )  # 根据配置查找资产的所有图片
        ...
        object_info = asset_id.split("-")[
            :-1
        ]  # 尝试从资产 ID 中提取对象信息（假设 ID 格式为 name-uuid）
        ...
        try:  # 尝试进行推理
            ...
            if (
                "json" in prompt_type.lower() or "extract" in prompt_type.lower()
            ):  # 如果提示词类型暗示需要结构化输出
                try:
                    result = self.parse_structured_text_enhanced(result_text)
                    if not result:
                        print(
                            f"[WARN] No structured data found for {asset_id}. Saving raw text."
                        )
                        result = {"raw_output": result_text}
                    else:
                        # Extract category from directory path and override
                        asset_relative_path = os.path.relpath(
                            asset_path, self.config.data.input_dir
                        )
                        ...
                        if result.get("dimensions"):
                            result["dimensions"] = self._normalize_dimensions(
                                result["dimensions"]
                            )
                        if result.get("mass"):
                    ...
                    print(
                        f"[WARN] Failed to parse structured text for {asset_id}: {e}. Saving raw text."
                    )
                    result = {"raw_output": result_text}
            else:  # 如果不需要结构化输出
        ...
        print(
            f"[INFO] Finished {asset_id} in {time.time() - start_time:.2f}s"
        )  # 打印处理完成及耗时信息
        return result  # 返回处理结果

    def _prepare_messages(
        self, user_prompt: str, image_paths: List[str]
    ) -> List[Dict[str, Any]]:  # 内部方法，准备消息列表
        content = []  # 初始化内容列表
        content.append({"type": "text", "text": user_prompt})  # 添加文本提示词

        for img_path in image_paths:  # 遍历所有图片路径
            content.append(
                {"type": "image_url", "image": img_path}
            )  # 添加图片 URL（路径）
