...
class ModelConfig:
    name: str  # 模型名称或路径
    backend: str = "local_hf"  # 模型后端，默认为本地 Hugging Face 推理
    api_base_url: Optional[str] = None  # API 后端基础地址
    api_key_env: Optional[str] = None  # API Key 对应的环境变量名
    api_timeout_seconds: int = 120  # API 请求超时时间（秒）
    api_max_retries: int = 2  # API 请求最大重试次数
    device_map: str = "auto"  # 设备映射策略，默认为 "auto"
    dtype: str = "bfloat16"  # 数据类型，默认为 "bfloat16"
    attn_implementation: str = (
        "flash_attention_2"  # 注意力机制实现，默认为 "flash_attention_2"
    )
